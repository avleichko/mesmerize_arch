...
- Known tensions to escalate (do not auto-close SAD Unknowns without a follow-on ADR): customer **D-01** Region `us-east-2` vs architecture **Q-07**; **D-04** one container per tenant vs ADR-013 Bridge; **D-05** Prod blue/green vs A-03 rolling; device transport **re-opened** vs ADR-007 Socket.io. **Repo layout:** customer **D-07** multi-repo and [ADR-017](017-python-platform-backend.md) **S8** are aligned (2026-08-19).

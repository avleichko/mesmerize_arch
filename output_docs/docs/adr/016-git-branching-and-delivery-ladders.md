...
3. **Do not claim Netlify or TTV filesync for Python/ECS platform services.** Ladder A must not be drawn or described as Netlify/TTV. ([ADR-017](017-python-platform-backend.md) supersedes NestJS as the API runtime.)  
...
5. **CI gate patterns (Proposed for platform):** Core GitHub Actions gate shape for **Python API** (ruff · mypy · pytest · image build) plus optional Node frontend jobs, PR template, and CODEOWNERS — stubs in [`docs/ci-templates/`](../ci-templates/). Sourced from Confirmed touchscreen-ux `.github` practices. Do **not** treat Ladder B–only workflows (whitelabel regen, content-link checks, contrast/axe, eringest probe, Netlify) as Ladder A requirements. Live platform workflow inventory remains **Unknown** until the customer monorepo adopts the stubs.
